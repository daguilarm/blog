...
La idea era no cargar nada de inicio, y en función de la selección ir cargando de forma dinámica un componente, tal y como se hubiera hecho con Jquery, cargando el resultado de una consulta `AJAX` en un contenedor `div`:
...
El sistema que planteo tiene un reto más, y es el de reutilizar componentes `Livewire` que ya he utilizado en otra parte del proyecto, y por tanto, no repetir código. Esto tiene un problema, y es que no puedo modificar (ni adaptar) el componente original que voy a cargar en el contenedor, ya que funciona perfectamente en su ubicación original.
...
Y obviamente no funcionaba. Obtenía el mensaje de error:
...
Lo cual está bien si los componentes son simples, pero si son complejos (como en mi caso) y encima son muchos componentes (doce componentes), te puedes encontrar con 87 llamadas a la base de datos... una situación poco ideal.

Otro problema que encontré es que cuando utilizas `x-if` tienes que utilizar la equiqueta `template`, o da error. Y al hacer esto, te obliga a tener un solo componente `root`, es decir, tienes que meter tu componente en un contenedor div principal:
...
El componente de arriba va a dar error, debemos cambiarlo por:
...
Después de revisar media internet en busca de soluciones, llegué a este comentario en github sobre el problema:

+ [https://github.com/livewire/livewire/pull/1534](https://github.com/livewire/livewire/pull/1534){.link-out}


Y proponía una solución como esta:
...
Esta técnica, lo que hace es reiniciar cada elemento Livewire, y al hacer esto, los elementos se van visualizando conforme hacemos click en cada enlace. 

En este punto, me puse a revisar el código `HTML` y efectivamente, todo el código se había eliminado del `DOM`... el problema lo encontré al revisar las consultas a la base de datos, seguían siendo 87. Es decir, por un lado eliminaba el `HTML` del `DOM` hasta que se hace la llamada a través del enlace, pero por otro seguía realizando las consultas a la base de datos. 

Seguía teniendo prácticamente el mismo problema que al principio.

Mi siguiente planteamiento fue el de crear un componente livewire que cada vez que pulsara en un enlace enviara una variable con el componente a cargar:
